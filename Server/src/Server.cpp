...
{
	std::u16string filePath = u"Monitors.kea";
...
void Server::saveMonitors()
{
	...
	writer.writeU16String(u"Karinov");
...
{
	std::u16string filePath = u"Scanners.kea";
	BinaryReader reader(filePath);
	if (!reader.isOpen())
		return;
...
void Server::startUp()
{
	base = std::shared_ptr<Base>(BaseLoader::load(u"Base.kea"));
	threats = std::make_shared<ThreatList>(u"Threats.kea");
	threats->load();
